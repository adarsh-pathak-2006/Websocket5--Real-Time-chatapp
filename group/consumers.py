...
class GroupChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.group_name=f"room_{self.user.id}"
    ...
    async def receive(self, text_data):
        ...
        if is_member and group_exists:
            user_obj = await User.objects.aget(id=sender_id)
            grp_obj = await Group.objects.aget(id=group_id)
            await GroupMessage.objects.acreate(sender=user_obj, message=message, group=grp_obj)
